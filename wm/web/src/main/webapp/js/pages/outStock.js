...
app.controller("outStockController",function ($scope,$http) {
    ...
    $scope.numYz = function () {
        ...
        if ($scope.goodsNum >= $scope.goodsMxaNum) {
    ...
    $scope.sureOutStock = function () {
        ...
        if ($scope.record.goods_name === null || $scope.record.num === null || $scope.record.repository_id === null || $scope.record.supplier === null || $scope.record.change_time === null || goodsWhValue === []) {
            alert("请填入完整信息");
            return;
        }
